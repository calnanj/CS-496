        ...
        if id:
            ...
            if boat_exists:
                ...
                if reset_slip:
                    ...
                    self.response.write("Success: boat deleted and slip emptied")
                else:
                    ndb.Key(urlsafe=id).delete()
                    self.response.write("Success: boat deleted")
            else:
                self.response.status = 400
                self.response.write("Error: that boat does not exist")
                ...
                if len(patch_boat_data) > 1:
                    ...
                    for key in patch_boat_data:
                        if key == "name":
                            ...
                            patch_boat.put()
                            self.response.write("Success: boat 'name' updated")
                        elif key == "type":
                            patch_boat.type = patch_boat_data['type']
                            patch_boat.put()
                            self.response.write("Success: boat 'type' updated")
                        elif key == "length":
                            patch_boat.length = patch_boat_data['length']
                            patch_boat.put()
                            self.response.write("Success: boat 'length' updated")
                        else:
                            self.response.status = 400
                            self.response.write("Error: the expected format is {\"name\": \"str\"} or {\"length\": int} or {\"type\": \"str\"}")
            else:
        ...
        if id:
            ...
            if boat_exists:
                ...
                if input_name and input_type and input_length:
                    ...
                    put_boat.put()
                    self.response.write("Success: boat 'name', 'type', and 'length' updated")
                else:
                    self.response.status = 400
                    self.response.write("Error: the expected format is {\"name\": \"str\", \"length\": int, \"type\": \"str\"}")
            else:
    ...
    def get(self, id=None):
        if id:
            ...
            if slip_exists:
                ...
                self.response.status = 400
                self.response.write("Error: that slip does not exist")
        else:
        ...
        if id:
            ...
            if slip_exists:
                ...
                if delete_slip.current_boat:
                    ...
                    self.response.write("Success: slip deleted and boat sent out to sea")
                else:
                    delete_slip.key.delete()
                    self.response.write("Success: slip deleted")
            else:
                self.response.status = 400
                self.response.write("Error: that slip does not exist")
                        ...
                        if key == "number":
                            ...
                            for slip in patch_slip_query_results:
                                if slip['number'] == patch_slip_data['number']:
                                    found_it = True
                                    self.response.status = 403
                                    self.response.write("Error: that slip number already in use")
                            if not found_it:
                                patch_slip.number = patch_slip_data['number']
                                patch_slip.put()
                                self.response.write("Success: slip 'number' updated")
                        else:
                            self.response.status = 400
                            self.response.write("Error: the expected format is {\"number\": int}")
            else:
                    ...
                    if boat_exists:
                        ...
                        if put_boat.at_sea:
                            if put_slip.current_boat == "":
                                ...
                                put_boat.put()
                                self.response.write("Success: boat brought in from sea and added to slip")
                            else:
                                self.response.status = 403
        ...
        if id:
            ...
            if slip_exists:
                ...
                if slip.current_boat:
                    ...
                    slip.arrival_date = ""
                    slip.put()
                    self.response.write("Success: boat removed from slip and sent out to sea")
                else:
                    self.response.status = 403
